...
internal unsafe partial struct ChunkedArray
{
    ...
    public ChunkEnumerator GetEnumerator() => new ChunkEnumerator(first);
    ...
    public struct ChunkEnumerator
    {
        ...
        public ChunkEnumerator(Chunk* first)
        {
            current = first;
            offset = 0;
        }

        public bool MoveNext(int maxBytes)
        {
            if (current == null)
                return false;

            while (current->Used - offset == 0)
            {
                current = current->Next;
                offset = 0;
                return MoveNext(maxBytes);
            }

            currentLength = Math.Min(maxBytes, current->Used - offset);
            offset += currentLength;
            return true;
        }

        public Span<byte> Current => current->GetSpan(offset - currentLength, currentLength);
    }
}
